...
#include "../../../beam-search/src/llama-arch_extend.h"
...
static std::set<llm_tensor> llm_get_tensor_names(llm_arch arch) {
    if (llm_arch_is_extend(arch)) {
        return llm_get_tensor_names_extend(arch);
    }
...
std::string LLM_KV::operator()(llm_kv kv) const {
    const char * arch_name = llm_arch_is_extend(arch) ? llm_arch_extend_name(arch) : llm_arch_name(arch);
...
std::vector<llm_arch> llm_arch_all() {
    ...
    ret.reserve(LLM_ARCH_NAMES.size() + 1);
    for (const auto & [arch, _] : LLM_ARCH_NAMES) {
        ret.push_back(arch);
    }
    ret.push_back(llm_arch_from_extend(LLM_ARCH_NLLB));
    return ret;
}
...
const char * llm_arch_name(llm_arch arch) {
    ...
    if (it == LLM_ARCH_NAMES.end()) {
        if (llm_arch_is_extend(arch)) {
            const char * extend_name = llm_arch_extend_name(arch);
...
llm_arch llm_arch_from_string(const std::string & name) {
    ...
    if (name == "nllb") {
        return llm_arch_from_extend(LLM_ARCH_NLLB);
    }

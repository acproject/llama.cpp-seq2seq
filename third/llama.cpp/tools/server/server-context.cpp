...
struct server_slot {
    ...
    bool awaiting_decoder_start = false;
    ...
    llama_token decoder_start_token() const {
        const llama_model * model = llama_get_model(ctx);
...
private:
    ...
    bool init() {
        ...
        // populate chat template params
        {
            ...
            const bool is_encoder_decoder = llama_model_has_encoder(model) && llama_model_has_decoder(model);
    ...
    void send_partial_response(server_slot & slot, const completion_token_output & tkn, bool is_progress) {
        ...
        if (is_progress) {
            ...
            res->progress.processed = slot.awaiting_decoder_start ? slot.task->n_tokens() : slot.prompt.tokens.size();
    ...
    void update_slots() {
        ...
        const bool model_is_encoder_decoder = llama_model_has_encoder(model) && llama_model_has_decoder(model);
        bool batch_has_decode_tokens = false;
        bool batch_has_prompt_tokens = false;

        if (model_is_encoder_decoder) {
            for (auto & slot : slots) {
                if (slot.state != SLOT_STATE_DONE_PROMPT || !slot.awaiting_decoder_start) {
                    continue;
                }
                ...
                std::vector<llama_token> decode_prefix;
                decode_prefix.push_back(slot.decoder_start_token());
                decode_prefix.insert(
                    decode_prefix.end(),
                    slot.task->params.decoder_prefix_tokens.begin(),
                    slot.task->params.decoder_prefix_tokens.end());

                for (size_t i_token = 0; i_token < decode_prefix.size(); ++i_token) {
                    const llama_token token = decode_prefix[i_token];
                    const bool logits = i_token + 1 == decode_prefix.size();
                    common_batch_add(batch, token, slot.prompt.tokens.pos_next(), { slot.id }, logits);
                    slot.prompt.tokens.push_back(token);
                }

                slot.i_batch = batch.n_tokens - 1;

                batch_has_decode_tokens = true;
            }
        }
        ...
        if ((params_base.cont_batching || batch.n_tokens == 0) && (!model_is_encoder_decoder || !batch_has_decode_tokens)) {
            for (auto & slot : slots) {
                if (!slot.is_processing()) {
                    continue;
                }
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                    ...
                    if (slot.state == SLOT_STATE_STARTED) {
                        ...
                        if (!slot.can_split()) {
                            ...
                            if (model_is_encoder_decoder && slot.task->n_tokens() > n_batch) {
                                send_error(slot,
                                           string_format(
                                               "input (%d tokens) is too large to encode in a single batch. increase the batch size (current batch size: %d)",
                    ...
                    if (model_is_encoder_decoder && slot.prompt.n_tokens() == 0 && slot.task->n_tokens() > n_batch - batch.n_tokens) {
                        continue;
                    }
                    ...
                    do_checkpoint = do_checkpoint && !model_is_encoder_decoder;
                    ...
                    batch_has_prompt_tokens = batch_has_prompt_tokens || n_tokens_cur > 0;
                    ...
                    if (slot.prompt.n_tokens() == slot.task->n_tokens()) {
                        ...
                        if (model_is_encoder_decoder) {
                            slot.awaiting_decoder_start = true;
                        } else {
        ...
        const bool batch_is_prompt_encode = model_is_encoder_decoder && batch_has_prompt_tokens && !batch_has_decode_tokens;

        SRV_DBG("%s batch, n_tokens = %d\n", batch_is_prompt_encode ? "encoding" : "decoding", batch.n_tokens);
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            const int ret = batch_is_prompt_encode ? llama_encode(ctx, batch_view) : llama_decode(ctx, batch_view);

            if (!batch_is_prompt_encode) {
            ...
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_DONE_PROMPT) {
                    ...
                    slot.awaiting_decoder_start = false;
...
std::unique_ptr<server_res_generator> server_routes::handle_completions_impl(
    ...
    auto & rd = res->rd;
    const bool model_is_encoder_decoder = ctx_server.model != nullptr &&
            llama_model_has_encoder(ctx_server.model) &&
            llama_model_has_decoder(ctx_server.model);
        ...
        for (size_t i = 0; i < inputs.size(); i++) {
            ...
            if (model_is_encoder_decoder) {
                if (task.params.n_cmpl > 1) {
                    throw std::runtime_error("encoder-decoder models do not support n_cmpl > 1 in server completion endpoints yet");
                }

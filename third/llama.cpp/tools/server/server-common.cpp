...
static std::string flatten_chat_msg_text(const common_chat_msg & msg) {
...
static std::string trim_encoder_decoder_text(std::string text) {
...
static std::string simplify_encoder_decoder_text(const std::string & text) {
    if (text.empty()) {
        return "";
    }
    ...
    if (std::regex_search(simplified, match, std::regex(R"(<text>\s*([\s\S]*?)\s*</text>)", std::regex::icase)) && match.size() > 1) {
        simplified = match[1].str();
    }

    simplified = std::regex_replace(simplified, std::regex(R"(<[^>]+>)"), " ");
    simplified = std::regex_replace(
        simplified,
        std::regex(R"(\btranslate the following text(?:\s+from\s+.+?)?(?:\s+to\s+.+?)?\s*[:.]?)", std::regex::icase),
        " ");
    simplified = std::regex_replace(
        simplified,
        std::regex(R"(\boutput the translation only\s*[:.]?)", std::regex::icase),
        " ");
    simplified = std::regex_replace(
        simplified,
        std::regex(R"(\breturn only the final translated text\s*[:.]?)", std::regex::icase),
        " ");
    simplified = std::regex_replace(simplified, std::regex(R"(\s+)"), " ");

    return trim_encoder_decoder_text(std::move(simplified));
}

static std::string build_encoder_decoder_prompt(const common_chat_templates_inputs & inputs) {
    std::string last_user_text;
    std::string last_non_system_text;

    for (const auto & msg : inputs.messages) {
        const std::string text = flatten_chat_msg_text(msg);
        if (text.empty()) {
            continue;
        }

        if (msg.role != "system") {
            last_non_system_text = text;
        }
    ...
    std::string prompt = simplify_encoder_decoder_text(!last_user_text.empty() ? last_user_text : last_non_system_text);
    if (!prompt.empty()) {
        return prompt;
    }
    ...
    std::string combined;
    for (const auto & msg : inputs.messages) {
        const std::string text = simplify_encoder_decoder_text(flatten_chat_msg_text(msg));
        if (text.empty()) {
            continue;
        }
        if (!combined.empty()) {
    ...
    return trim_encoder_decoder_text(std::move(combined));
}

static common_chat_params build_content_only_chat_params(const common_chat_templates_inputs & inputs) {
    if (!inputs.tools.empty()) {
        throw std::invalid_argument("tool calls require an explicit chat template for encoder-decoder models");
    }

    common_chat_params params;
    params.prompt = build_encoder_decoder_prompt(inputs);

    if (params.prompt.empty()) {
        throw std::invalid_argument("chat messages do not contain any textual content");
    }
    ...
    std::vector<raw_buffer> & out_files)
{
    ...
    if (!opt.has_explicit_template && opt.is_encoder_decoder) {
        SRV_WRN("%s: encoder-decoder model has no explicit chat template, falling back to content-only chat formatting\n", __func__);
        chat_params = build_content_only_chat_params(inputs);
    } else {

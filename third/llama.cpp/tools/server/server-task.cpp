...
static std::string normalize_encoder_decoder_target_lang(std::string value) {
    value = trim_copy(std::move(value));
    if (value.empty()) {
        return "";
    }

    if (value.size() > 4 && value.rfind("__", 0) == 0 && value.substr(value.size() - 2) == "__") {
    ...
    const auto alias = aliases.find(lowercase_copy(value));
...
json task_params::to_json(bool only_metrics) const {
    ...
    if (only_metrics) {
        return json {
            ...
            {"target_language",           encoder_decoder_target_lang},
            {"samplers",                  samplers},
...
task_params server_task::params_from_json_cmpl(
    ...
    {
        std::string target_language = json_value(data, "target_language", json_value(data, "target_lang", std::string("zho_Hans")));
        target_language = normalize_encoder_decoder_target_lang(std::move(target_language));

        if (!target_language.empty()) {
            auto ids = common_tokenize(vocab, target_language, /* add_special = */ false, /* parse_special = */ true);
            if (ids.size() != 1) {
                throw std::runtime_error("Error: target_language must map to exactly one token, for example zho_Hans or eng_Latn");
            }

            params.encoder_decoder_target_lang = std::move(target_language);
            params.decoder_prefix_tokens.push_back(ids[0]);
            SRV_DBG("Encoder-decoder target_language: %s -> %d\n", params.encoder_decoder_target_lang.c_str(), ids[0]);
        }
    }

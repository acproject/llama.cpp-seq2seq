...
void llama_model::load_arch(llama_model_loader & ml) {
    ...
    if (arch == LLM_ARCH_UNKNOWN) {
        const std::string arch_name = ml.get_arch_name();
        if (arch_name == "nllb") {
            arch = llm_arch_from_extend(LLM_ARCH_NLLB);
        } else {
            throw std::runtime_error("unknown model architecture: '" + arch_name + "'");
        }
    }

    ml.llm_kv = LLM_KV(arch);
}
...
std::string llama_model::arch_name() const {
    if (llm_arch_is_extend(arch)) {
        return llm_arch_extend_name(arch);
    }
...
bool llama_model_has_encoder(const llama_model * model) {
    switch (model->arch) {
        ...
        case LLM_ARCH_NLLB:      return true;
